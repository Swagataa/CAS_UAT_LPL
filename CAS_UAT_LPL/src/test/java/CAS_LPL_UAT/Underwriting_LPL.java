...
public class Underwriting_LPL extends BrowserSetup{

	static String app_id;	
	static WebDriverWait wait;
	static String ApplicationIdsfilepath1 = "C:\\Automation_Softwares\\eclipse\\WorkSpace\\CAS_UAT_LPL\\src\\test\\java\\testdata\\ApplicationIds.xlsx";

	@Test(dataProvider="data-provider",dataProviderClass=utility.DataProvider.class)
	public void Underwriting(String sLogin_id,String sPassword,String sArea,String First_Name,String Last_Name,String Father_Name,String Product,
...
//		driver.manage().timeouts().implicitlyWait(100, TimeUnit.SECONDS);
	...
	//	searchappId.sendKeys(app_id);
		...
		WebElement view_file = driver.findElement(By.name("B12"));
		
		wait.until(ExpectedConditions.visibilityOf(view_file));
		...
		List<String> view_filewindow = new ArrayList<String>(driver.getWindowHandles());

		Collections.sort(view_filewindow);

		System.out.println(view_filewindow);

		driver.switchTo().window(view_filewindow.get(3));

		System.out.println(driver.getTitle());

		if (driver.getTitle().equalsIgnoreCase("Finnone CAS")) {

			driver.switchTo().window(view_filewindow.get(2));

			System.out.println(driver.getTitle());

			if (driver.getTitle().equalsIgnoreCase("Finnone SSO")) {

				driver.switchTo().window(view_filewindow.get(1));

				System.out.println(driver.getTitle());

				if (driver.getTitle().equalsIgnoreCase("UnderWriter")) {

					driver.switchTo().window(view_filewindow.get(0));
		...
		WebElement verify = driver.findElement(By.name("selVerify"));
		
		wait.until(ExpectedConditions.visibilityOf(verify));
		
		verify.click();
		
		WebElement close_btn = driver.findElement(By.xpath("//input[@Value='Close']"));
		...
		/*------------ Return From View file window -------- */
		
		List<String> returnreportwindow = new ArrayList<String>(driver.getWindowHandles());

		Collections.sort(returnreportwindow);

		System.out.println(returnreportwindow);

		driver.switchTo().window(returnreportwindow.get(2));

		System.out.println(returnreportwindow.get(2));

		System.out.println(driver.getTitle());

		if (driver.getTitle().equalsIgnoreCase("Finnone CAS")) {

			driver.switchTo().window(returnreportwindow.get(0));

			System.out.println(driver.getTitle());

			if (driver.getTitle().equalsIgnoreCase("Finnone SSO")) {

				driver.switchTo().window(returnreportwindow.get(1));
...
//		driver.switchTo().window(FIreportwindow.get(0));
		...
		WebElement dedupeviewfilebtn = driver.findElement(By.xpath("//form[@name='underWriterAF']/table[16]//tbody[1]/tr[3]/td[2]/input[1]"));
...
//		driver.switchTo().window(dedupeviewfilewindow.get(0));
	...
	//	WebElement cibilviewfilebtn = driver.findElement(By.name("B15"));
	...
	/*	WebElement cibilclosebtn = driver.findElement(By.name("btnClose"));

		wait.until(ExpectedConditions.visibilityOf(cibilclosebtn));
